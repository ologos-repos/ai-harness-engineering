...
**Second realization (different substrate).** [`model-construct/grok-console/07-policy-enforcement.md`](../../model-construct/grok-console/07-policy-enforcement.md) maps the same requirement set onto a structurally different mechanism — a deny-glob catalog consumed by the model CLI's own native permission engine, rather than a wrapping hook — and surfaces a candidate additional requirement (an explicit conflict-priority rule for competing obligations) not yet in this clause's control set.
