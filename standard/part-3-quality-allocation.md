...
**Limit of the test: intent-ambiguous incidents.** Steps 2–3 above presuppose an authoritative *intended target* to compare the model's output and the harness's context against. That premise holds when intent was unambiguous — the kind of incident real-target resolution (→ §7 PE-03) should already have caught pre-execution. It does not hold for the harder subclass where the divergence exists *because* the original intent was genuinely underspecified: there is no ground truth in the evidence to classify against, because none was ever established. This is not a gap in the test's design; it is the same limit Part 4's R4 residual names directly — "structure validates existence and permission, not correspondence to intent." §3.4.2's AHES-QA-06 gives this subclass its own disposition rather than forcing it into "allocation underspecified" or "evidence insufficient."

### 3.4.2 Requirements

- **AHES-QA-05** — A conforming deployment **shall** maintain a postmortem template that assigns an M / H / S / O classification to any failure using **only** captured evidence (→ §10 Evidence engineering), without recourse to the reasoning transcript of the review itself, undocumented operator recollection, or re-execution. If the classification cannot be reached from captured evidence, the template **shall** distinguish two cases (→ AHES-QA-06): the evidence architecture did not capture what it should have, or the evidence architecture captured everything it structurally could and the incident's underlying intent was never established well enough for any evidence to settle it.

- **AHES-QA-06** — Inability to classify a failure to at least one locus within a single review pass **shall** be recorded as one of two dispositions, not conflated: **(a) a Part 3 conformance defect** — the allocation for the affected characteristic was underspecified (AHES-QA-01) or its evidence was insufficient given what *could* have been captured (AHES-QA-05) — which **shall** drive a corrective update to the allocation record or the evidence architecture; or **(b) an intent-ambiguity finding** — the incident's ground-truth intent was never established at the time of action, so no evidence architecture could have settled the classification, per the limit named in §3.4.1. Disposition (b) **shall not** be recorded as an evidence-architecture defect, but **shall** drive a corrective update to *intent-capture* upstream (e.g., a §9 principal-authority scoping gap, or an under-specified request the harness should have clarified before acting) rather than being silently absorbed as review friction.
